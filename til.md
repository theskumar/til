...
- 05 Sep 2026. Every coding agent (Claude Code, Codex, pi) compacts long sessions the same way: ask an LLM to prose-summarize, then summarize that summary, until load-bearing details erode (why a decision was made, which approaches got rejected). [pi-blackhole](https://github.com/k0valik/pi-blackhole) swaps that for deterministic compaction: a zero-token `compile()` that extracts structured sections (goal, files touched, commits, preferences, brief transcript). No LLM, no drift, no cost. The compile is only lossy in the window, not on disk: blackhole also ships a `recall` tool (BM25 + regex over the raw session JSONL and the memory ledger) that re-fetches any dropped turn on demand, so compile + recall together is lossless where an LLM summary-of-a-summary destroys the original. It also adds observational memory: background observer/reflector workers append durable facts to per-session ledgers that survive every compaction, and `/blackhole-export` walks every past-session JSONL under a git-root into a ranked context file. The clever part is what export trusts. A fact's rank is driven by cross-session *recurrence*, damped as `log(1 + distinctSessions)` with a burst penalty, not raw counts, so one chatty session can't inflate it. Recurrence-as-reinforcement is basically [Hebbian](https://saurabh-kumar.com/articles/2025/11/notes-on-understanding-hebbian-learning/): facts that keep firing across sessions get stronger. Dedup is a 3-pass funnel, exact then Levenshtein then Sørensen-Dice token-set, with a Levenshtein floor so keyword overlap alone can't merge two distinct facts. Bonus: the OM workers run on their own model and billing, so I point them at `openai-codex/gpt-5.6-luna` while the main thread stays on Claude. #ai #tools [discuss](https://mastodon.social/@theskumar/117217821983457111)
# Aug 2026

- 04 Aug 2026. Someone finally named it: [meat proxy](https://gruhn.me/blog/2026-08-03/). You paste Claude's answer into Slack or a PR review without reading it, and you have made yourself the pipe between me and a model I could have prompted myself, with better context. Everyone already knows that annoyance. Nobody had the word for it. The word is the useful part, because "don't be a meat proxy" is something you can actually say to a colleague: it names a role instead of calling them lazy. Best line is about code review. Paste the ticket into Claude, paste the reviewer feedback back in, read neither, and the reviewers implemented your PR while you held the pipe. The bar before forwarding anything is to write it in your own words, "a decent certificate that you've done the prior steps." 704 comments on [HN](https://news.ycombinator.com/item?id=49151933), barely any disagreement. #ai

# Jul 2026

- 28 Jul 2026. Python's `Ellipsis` literal (`...`) is a real object (`type(...)` is `<class 'ellipsis'>`), not just a placeholder. Beyond type stubs and numpy slicing, [NVIDIA's NOOA framework](https://github.com/NVIDIA-NeMo/labs-OO-Agents) repurposes it as a runtime dispatch marker: methods with `...` bodies are completed by an LLM at runtime, while methods with real bodies stay deterministic Python. The boundary between "code I control" and "code the model controls" becomes greppable: `grep -n '\.\.\.' agent.py`. Clever reuse of existing syntax that linters and type checkers already accept. #python #ai [discuss](https://mastodon.social/@theskumar/116993643079283240)

- 24 Jul 2026. [django-crawl](https://django-crawl.readthedocs.io/en/latest/) by Adam Johnson: BFS-crawls your Django site using the test client, no real HTTP overhead. [Origin story](https://adamj.eu/tech/2026/07/22/introducing-django-crawl/): diffing CSP headers during a Django 6.0 migration accidentally caught 7 non-CSP bugs in a 100%-coverage project. HTML parsing via Rust (html5ever), follows links/sitemaps/feeds. Works as a management command or a TestCase smoke test that raises `ExceptionGroup` on errors. #django #python [discuss](https://mastodon.social/@theskumar/116974727241984366)
